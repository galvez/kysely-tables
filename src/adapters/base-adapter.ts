...
export abstract class BaseDatabaseAdapter implements DatabaseAdapter {
  ...
  constructor(tables: TableDefinition[], databaseType: import('../types').DatabaseType) {
    this.tables = tables;
  ...
  protected getTableNameFromInterface(interfaceName: string): string {
    const withoutSuffix = interfaceName.replace(/Table$/, '');
    return this.convertNameToSnakeCase(withoutSuffix);
  }
